...
def get_average_sentence_length(text):
    ...
    sentence_lengths = []
    for i in sentences:
        sentence_lengths.append(i.count(' ') + 1) 
    average_length = int(sum(sentence_lengths) / len(sentence_lengths)) # rounds numbers
...
class TextSample():
    ...
    def __repr__(self):
        return "{}, average sentence length: {} words".format(self.author, self.average_sentence_length)
...
def prepare_text(text):
    ...
    text = text.replace('.', '') # removes fullstops
    text = text.replace('?', '') # removes question marks
    text = text.replace('!', '') # removes exclaimation points
...
def build_frequency_table(corpus):
    frequency_table = {}
    unique_words = set(corpus)  # list unique words in corpus
...
def ngram_creator(text_list):
    ngram = []
    for i in range(0, len(text_list) - 1): # range to go to second to last word in text
...
def frequency_comparison(table1, table2):
    ...
    return mutual_appearances / appearances # freq comparison score
...
def find_text_similarity(textsample1, textsample2):
    ...
    return "%.3f" % similarity_score  # return similarity_score rounded to 3 decimal places
...
for contestant in contestants:
    print('name: {}, similarity to murder letter: {}'.format(contestant.author, find_text_similarity(murderer_sample, contestant)))
